fn main() {
    ...
    let rows = arg_2d.len();
    let cols = arg_2d[0].len();
    let mut adjacent_neighbors = 4;
    let mut products: Vec<i32> = Vec::new();
    println!("element = {}", arg_2d[6][8]);
    for i in 0..rows{
        for j in 0..cols {
            if i + adjacent_neighbors - 1  < rows {
                // Can get a row slice
                let mut slice = Vec::new();
                for k in 0..adjacent_neighbors {
                    if i==6 && j==8 {
                    println!("elements = {}", arg_2d[i+k][j]);
                    }
                    slice.push(arg_2d[i + k][j]);
                }
                let product: i32 = slice.iter().cloned().product();
            ...
            if j + adjacent_neighbors -1 < cols {
                // Can get a column slice
                let mut slice = Vec::new();
                for k in 0..adjacent_neighbors {
                    if i==6 && j==8 {
                    println!("elements = {}", arg_2d[i][j+k]);
                    }
                    slice.push(arg_2d[i][j+k]);
                }
                let product: i32 = slice.iter().cloned().product();
                products.push(product);
                if i==6 && j==8 {
            ...
            /*
                A
                    B
                        C
             */
            if i + adjacent_neighbors -1 < rows && j + adjacent_neighbors -1 < cols {
                // Can get a diagonal slice
                let mut slice = Vec::new();
                for k in 0..adjacent_neighbors {
                    if i==6 && j==8 {
                    println!("elements = {}", arg_2d[i+k][j+k]);
                    }
                    slice.push(arg_2d[i+k][j+k]);
                }
                let product: i32 = slice.iter().cloned().product();
                products.push(product);
                if i==6 && j==8 {
            ...
            /*
                    ...
                    C
             */
            if i + adjacent_neighbors < rows && ((j as i32) - (adjacent_neighbors as i32) > -1) {
                // Can get a diagonal slice
                let mut slice = Vec::new();
                for k in 0..adjacent_neighbors {
                    if i==6 && j==8 {
    ...
    println!("largest product = {:?}", products.iter().max())
}
